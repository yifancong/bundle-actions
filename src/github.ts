...
export class GitHubService {
  ...
  async getTargetBranchLatestCommit(): Promise<string> {
    ...
    try {
      ...
      return execSync(`git rev-parse --short=10 origin/${targetBranch}`, { encoding: 'utf8' }).trim();
    } catch (error) {
      console.warn(`Failed to get target branch commit from workflow runs, falling back to git: ${error}`);
      // Fallback: get commit hash directly from git
      return execSync(`git rev-parse --short=10 origin/${targetBranch}`, { encoding: 'utf8' }).trim();
    }
  }
