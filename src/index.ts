...
(async () => {
  try {
    ...
    if (isMergeEvent()) {
      ...
      try {
        if (baselineJsonPath) {
          ...
          try {
            ...
            runLink = `${process.env.GITHUB_SERVER_URL}/${process.env.GITHUB_REPOSITORY}/actions/runs/${process.env.GITHUB_RUN_ID}`;
      ...
      // Add comment to PR if this is a pull request (even without baseline)
      if (isPullRequestEvent()) {
        ...
        if (runLink) {
          commentBody += `A detailed bundle diff analysis has been generated using Rsdoctor. You can download and view the interactive HTML report from the workflow artifacts.\n\n`;
          commentBody += `📦 **Download Link**: [Download Bundle Diff Report](${runLink})\n\n`;
        } else {
          commentBody += `No baseline data found. This is the first bundle analysis for this path.\n\n`;
        }
        
        commentBody += `${bundleAnalysisMarkdown}\n\n`;
        commentBody += `*Generated by Bundle Size Action*`;
